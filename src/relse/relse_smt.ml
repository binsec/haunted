...
module Solver = struct
  ...
  let prepare_solver_in_state ?keep ~retire_stl ps assertions solver qtype =
    ...
    let final_formula =
      let fml = Formula.push_front_comment "Assertions to check"
          (Path_state.formula ~retire_stl ps) in
  ...
  let with_solver ?keep ~retire_stl qtype ps formulas f =
    ...
    let prepare_solver = prepare_solver_in_state ?keep ~retire_stl ps formulas in
    try
  ...
  let check_sat_formulas ~retire_stl qtype formulas path_state  =
    let rec check_sat_formulas qtype formulas path_state =
      ...
      in
      let result, to_load =
        match with_solver ~retire_stl qtype path_state formulas do_check with
        | Some (CheckSat (result, to_load)) -> result, to_load
        | _ -> Formula.UNKNOWN, no_address
      in
      maybe_load_and_recurse
        (check_sat_formulas qtype formulas)
        result path_state to_load
    in
    let result, ps = check_sat_formulas qtype formulas path_state in
    Relse_stats.update_status result (Path_state.get_current_statement ps) qtype;
    result, ps
  
  let check_sat_pc ~retire_stl ps =
    match Path_state.path_constraint ps with
    | Relse_utils.Valid ->
      Logger.debug ~level:9 "[Solver][check_sat] Spared pc check: Valid";
      Formula.SAT, ps
    | Relse_utils.Unsat ->
      Logger.debug ~level:9 "[Solver][check_sat] Spared pc check: Unsat";
      Formula.UNSAT, ps
    | Relse_utils.Sat _ ->
      Logger.debug ~level:9 "[Solver][check_sat] Spared pc check: Sat";
      Formula.SAT, ps
    | Relse_utils.Unknown pc ->
      let status, ps = check_sat_formulas ~retire_stl Relse_stats.Exploration [pc] ps in
      let ps = match status with
        | Formula.SAT -> Path_state.mark_sat ps
        | Formula.UNSAT -> Path_state.mark_unsat ps
        | Formula.TIMEOUT | Formula.UNKNOWN -> Path_state.mark_unknown ps
      in status, ps

      
  let get_model_formulas ~retire_stl formula path_state = (* TODO: remove, UGLY *)
    let do_check session =
      let result = Solver.Session.check_sat session in
      let model =
        match result with
      ...
      in GetModel model
    in
    match with_solver ~retire_stl Relse_stats.Model path_state formula do_check with
      | Some (GetModel m) -> m
      | _ -> failwith "We ask the solver a model when formula is unsat"

  let get_model_pc ~retire_stl ps =
    match Path_state.path_constraint ps with
    | Relse_utils.Valid ->
      get_model_formulas ~retire_stl [Formula.mk_bl_true] ps
    | Relse_utils.Unsat ->
      get_model_formulas ~retire_stl [Formula.mk_bl_false] ps
    | Relse_utils.Sat pc ->
      get_model_formulas ~retire_stl [pc] ps
    | Relse_utils.Unknown pc ->
      get_model_formulas ~retire_stl [pc] ps

  let rec enumerate_values ~retire_stl n expr pc path_state =
    ...
    let values, to_load =
      match with_solver ~keep ~retire_stl Relse_stats.Exploration
              path_state [pc] (loop [] no_address n) with
      | Some (Enum (values, to_load)) -> values, to_load
      | _ -> [], no_address in
    let values, path_state =
      maybe_load_and_recurse (enumerate_values ~retire_stl n expr pc) values path_state to_load
    in
...
module Translate = struct
  ...
  let rec expr ?(high=false) (* ?(iv=true) *) ps e = (* RELSE *)
    let smt_unary = unary and smt_binary = binary in
    let open Dba.Expr in
    let open Rel_expr in
    match e with
    | Var { name; size = bitsize; _ } ->
      Relse_term.TransientSet.create (Path_state.var_load ps name (Size.Bit.create bitsize))

    | Cst (_, bv) -> Relse_term.TransientSet.create (mk_simple (Formula.mk_bv_cst bv))

    | Load (bytes, _endianness, index) ->
      (* It seems that the weak untaint here makes the performances
         worse *)
      let index = (expr ps index) in
      Relse_term.TransientSet.fail_if_transient index;
      let r_index = Relse_term.TransientSet.regular index in
      Path_state.memory_select ps bytes r_index

    | Binary (bop, op1, op2) as e ->
      Logger.debug ~level:6 "Translating binary %a" Dba_printer.Ascii.pp_bl_term e;
      let rel_e1 = expr ps op1 ~high
      and rel_e2 = expr ps op2 ~high in
      let do_binary e1 e2 =
        apply2 (smt_binary bop) e1 e2
      in Relse_term.TransientSet.cartesian do_binary rel_e1 rel_e2

    | Unary (uop, e) ->
      let rel_e = expr ps e ~high
      in Relse_term.TransientSet.map (apply (fun x -> smt_unary e uop x)) rel_e

    | Ite (c, then_e, else_e) ->
      let rel_c = expr ps c ~high
      and rel_then = (expr ps then_e ~high) 
      and rel_else = (expr ps else_e ~high) in
      let do_ite r_c r_then r_else =
        Rel_expr.apply3 (fun c e_then e_else ->
            Formula.(mk_bv_ite (mk_bv_equal c (mk_bv_one)) e_then e_else))
          r_c r_then r_else
      in
      Relse_term.TransientSet.cartesian3 do_ite rel_c rel_then rel_else

  let expr_noset ?(high=false) ps e =
     let e = expr ~high ps e in        
     Relse_term.TransientSet.fail_if_transient e;
     Relse_term.TransientSet.regular e
  ...
  (** [lvalue_with_rval_update ps rel_val high] Returns
      the function that updates [rval] in [ps]. If
      [high=true], any undeclared variable is declared high, otherwise,
      it is low *)
  let lvalue_with_rval_update ps rel_val =
    function
    ...
    | LValue.Restrict ({name; size = bitsize; _}, {Interval.lo; Interval.hi}) ->
      let size = Size.Bit.create bitsize in
      let rel_var = Path_state.var_load ps name size in
    ...
    | LValue.Store (size, _, e) ->
      let r_index = expr ps e in
      Relse_term.TransientSet.fail_if_transient r_index;
      let r_index = Relse_term.TransientSet.regular r_index in
      Relse_utils.Mem (size, r_index, rel_val) 

  let assignment ?(init=false) lvalue rvalue ps =
    Path_state.assign ~init (lvalue_with_rval_update ps rvalue lvalue) ps
end
