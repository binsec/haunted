...
let lhs_of_flag f inst =
  let bitsize = match f with
    | X86Types.IOPL -> Size.Bit.create 2
    | _ -> Size.Bit.bits1 in
  Dba.LValue.var (X86Util.flag_to_string f)
    ~bitsize ~tag:(Dba.VarTag.flag inst)
...
let expr_of_flag f =
  let bits = match f with
    | X86Types.IOPL -> Size.Bit.create 2
    | _ -> Size.Bit.bits1 in
  Dba_types.Expr.flag (X86Util.flag_to_string f) ~bits
...
let iopl_flag = expr_of_flag IOPL
...
let cpl_register = Dba.Expr.var X86Util.cpl_to_string 2
let check_iopl_ge_cpl = Predba.dynamic_assert (Dba.Expr.uge iopl_flag cpl_register)
...
let instruction_to_dba rep sreg nextaddr opcode instruction =
  ...
  | Mfence -> [ Predba.serializing Dba.SerializeMemory ]
  ...
  (* Warning: instructions not fully implemented ! Missing cases. *)
  | Cli -> check_iopl_ge_cpl :: [ assign_flag IF Dba.Expr.zero ]
  | Sti -> check_iopl_ge_cpl :: [ assign_flag IF Dba.Expr.one ]
