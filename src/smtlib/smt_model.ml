...
type t = {
  ...
  booleans : bool Basic_types.String.Htbl.t;
}

let create ?(len=43) () = {
  memory = Memory.empty ();
  variables = Basic_types.String.Htbl.create len;
  booleans = Basic_types.String.Htbl.create len;
}
...
let add_bool t name bl =
  Basic_types.String.Htbl.replace t.booleans name bl
...
let pp ppf t =
  ...
  and pp_booleans ppf r =
    if Stbl.length r > 0 then begin
        pp_print_string ppf "# Booleans";
        pp_print_cut ppf ();
        let l =
          Basic_types.String.Htbl.fold (fun k v acc -> (k, v) :: acc) r []
          |> List.sort (fun (k1, _) (k2, _) -> String.compare k1 k2)
        ...
        List.iter
          (fun (name, bl) ->
            fprintf ppf "%s : %s@ " name (if bl then "True" else "False")) l;
        pp_print_cut ppf ();
      end
  ...
  else fprintf ppf "@[<v 0>--- Model ---@,%a@,%a@,%a@]"
      pp_variables t.variables
      pp_booleans t.booleans
      pp_memory t
...
let filter ?(keep_default=false) ~addr_p ~var_p t =
  ...
  { variables; memory; booleans = t.booleans; }
;;
...
(* let value_of_bool bl =
 *   if String.compare bl "false" = 0 then false
 *   else if String.compare bl "true" = 0 then true
 *   else
 *     begin
 *       Logger.error
 *         "Model construction: unexpected constant %s as boolean value" bl;
 *       exit 2
 *     end *)
...
let extract_boolean term =
  match term.term_desc with
  | TermSpecConstant (CstBool bl) -> bl
  | _  -> assert false
...
let get_boolean cmd =
  ...
  | CmdDefineFun fdef ->
    begin
      match fdef.fun_def_desc with
      | FunDef (symbol, _, _, _, term) ->
        let value = extract_boolean term in
        symbol, value
    end
  | _ -> assert false
...
let add_boolean smt_model cmd =
  let symbol, value = get_boolean cmd in
  let name = get_symbol_name symbol in
  Logger.debug ~level:2
    "Add boolean %s as bl %B" name value;
  add_bool smt_model name value
...
let is_boolean sort =
  ...
  | SortIdentifier identifier ->
    begin
      match identifier.id_desc with
      | IdSymbol symbol ->
        if String.compare (get_symbol_name symbol) "Bool" = 0
        then true
        else false
      | _ -> false
    end
...
let is_bl_variable cmd =
  match cmd.command_desc with
  | CmdDefineFun fdef ->
    begin
      match fdef.fun_def_desc with
      | FunDef (_, optargs, args, sort, _) ->
        optargs = None && args = [] && is_boolean sort
    end
  | _ -> assert false
...
let extract model_ast =
  ...
  let booleans, functions = 
    List.partition is_bl_variable functions in
  Logger.debug ~level:2 "Found : %d booleans, %d functions"
    (List.length variables) (List.length functions);
  List.iter (add_boolean smt_model) booleans;
